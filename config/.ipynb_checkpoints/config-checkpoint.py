...
@dataclass
class TLOB(Model):
    hyperparameters_fixed: dict = field(default_factory=lambda: {"num_layers": 4, "hidden_dim": 40, "num_heads": 1, "is_sin_emb": True, "lr": 0.0002, "seq_size": 128, "all_features": True})
...
@dataclass
class RMoney_NSE(Dataset):
    type: DatasetType = DatasetType.RMoney_NSE
    dates: list = field(default_factory=lambda: ["2025-08-28", "2025-08-29"])
    sampling_type: SamplingType = SamplingType.NONE
...
@dataclass
class Custom_5_days(Dataset):
    type: DatasetType = DatasetType.Custom_5_days
    dates: list = field(default_factory=lambda: ["2025-08-18", "2025-08-22"])
    sampling_type: SamplingType = SamplingType.NONE
    batch_size: int = 64
    training_stocks: list = field(default_factory=lambda: ["INFY"])
    testing_stocks: list = field(default_factory=lambda: ["INFY"])
...
class Experiment:
    ...
    seed: int = 1
    horizon: int = 500  # Chnaged for custom use. HORIZON SHOULD BE GREATER THAN window_length
...
cs.store(group="dataset", name="custom_5_days", node=Custom_5_days)