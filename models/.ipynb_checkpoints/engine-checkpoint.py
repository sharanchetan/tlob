...
class Engine(LightningModule):
    def __init__(
        ...
        try:
            # If labels are available in the dataloader later, you can compute dynamically.
            # For now, we’ll check if precomputed weights are passed from run.py
            if hasattr(self, "class_weights") and self.class_weights is not None:
                self.loss_function = nn.CrossEntropyLoss(weight=self.class_weights.to(cst.DEVICE))
                print("Cow Says: Error Weights Applied! Moo!")
            else:
                self.loss_function = nn.CrossEntropyLoss()
        except Exception as e:
            print(f"[Warning] Failed to apply class weights: {e}")
            self.loss_function = nn.CrossEntropyLoss()
    ...
    def configure_optimizers(self):
        ...
        if self.optimizer == 'Adam':
            self.optimizer = torch.optim.Adam(self.parameters(), lr=self.lr, eps=eps, weight_decay=1e-5)
    ...
    def model_checkpointing(self, loss):        
        ...
        if self.last_path_ckpt is not None and os.path.exists(self.last_path_ckpt):
            try:
                os.remove(self.last_path_ckpt)
            except Exception as e:
                print(f"[Warning] Could not remove previous checkpoint {self.last_path_ckpt}: {e}")
