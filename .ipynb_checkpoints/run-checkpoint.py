...
from preprocessing.custom_5_day import custom_5_days_load # Added for custom use.
...
def train(config: Config, trainer: L.Trainer, run=None):
    ...
    seq_size = config.model.hyperparameters_fixed["seq_size"]
    horizon = config.experiment.horizon
    window_length = config.experiment.window_length  # Added for custom use.
    ...
    elif dataset_type == "RMoney_NSE":  # Added for custom use.
    ...
    elif dataset_type == "Custom_5_days":  # Added for custom use.
        path = cst.CUSTOM_5_DAY + "/custom_5_days/"
        train_input, train_labels, val_input, val_labels, test_input, test_labels = custom_5_days_load(path, window_length, horizon)   
        train_set = Dataset(train_input, train_labels, seq_size)
        val_set = Dataset(val_input, val_labels, seq_size)
        test_set = Dataset(test_input, test_labels, seq_size)
        if config.experiment.is_debug:
    ...
    # Extract class counts from training labels
    class_counts = counts_train[1].float()
    num_classes = len(class_counts)
    total_samples = class_counts.sum()

    # Inverse frequency weighting (standard approach)
    class_weights = total_samples / (num_classes * class_counts)

    # Normalize so average weight = 1 (optional but stable)
    class_weights = class_weights / class_weights.mean()
    class_weights = class_weights.to(cst.DEVICE)

    print(f"Computed class weights (for imbalance correction): {class_weights}")
...
def run_wandb(config: Config, accelerator):
    def wandb_sweep_callback():
        ...
        trainer = L.Trainer(
            ...
            callbacks=[
            EarlyStopping(monitor="val_loss", mode="min", patience=12, verbose=True, min_delta=0.002),
